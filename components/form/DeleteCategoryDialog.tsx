...
function DeleteCategoryDialog({ categoryId, onSubmitSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${BASE_API_URL}/api/category`, {
        ...
        body: JSON.stringify({ userId: userId, categoryId }),
      });
    ...
    <Dialog open={openDialog} onOpenChange={setOpenDialog}>
      ...
      <DialogContent>
        <DialogHeader className="flex items-center space-y-4 max-md:mt-6">
          ...
          <DialogDescription>
            This action will delete this category and all of its history
            permanently.
          </DialogDescription>
        </DialogHeader>
